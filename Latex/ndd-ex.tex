...
passes through the gas it ionizes the gas atoms and the electrons and ions are collected at the electrodes of the detector. Since the created 
electron-ion pairs are hard to detect we use the Townsend avalanche effect to amplify the signal, which gives us a cascade of 
electron-ion pairs. Without multiplication of the signal the energy resolution is given as:
%
\begin{equation}
    N = \frac{\Delta E}{w} = \sqrt{\frac{F\cdot w}{\Delta E}}\>,
    \label{eq:ionization-res}
\end{equation}
%
where $F$ is the Fano factor which is around $F = 0.2$ for gas ionization chambers. The Fano factor is needed to correct the fact that 
subsequent electron-ion pairs are not entirely statistically independent. $w$ is the energy needed to create an electron-ion pair in the
gas. For argon $Ar$ it is $w_\text{Ar} = 26\,\rm{eV}$. The amount of charge we collect is:
%
\begin{align}

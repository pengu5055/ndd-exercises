...
% Define Color
\definecolor{box-cover-color}{RGB}{144,12,63}
\definecolor{dots-cover-color}{RGB}{54,239,216}
\definecolor{dots-cover-border-color}{RGB}{20,90,81}
...
\begin{figure}
    \centering
    \begin{tikzpicture}[overlay, remember picture,opacity=0.7]
...
{\small\noindent 
    I'd like to thank my colleagues Tinka D., Gaja Š. and Marko M. for their help in sourcing notes for the exercises I missed.
    With out them I would not have been able to complete this document. 
}
\newpage
...
%
\begin{equation}
    \xi = \frac{K}{2}\left\langle\frac{Z}{A}\right\rangle\frac{x}{\beta^2}\>,
    \label{eq:xi}
\end{equation}
%
...
\end{flalign}
%
\subsubsection{What is the estimate maximum energy transfer in one collision of a WIMP with a germanium nucleus?}
...
%
\begin{flalign}
    p_T &= e_0\cdot 1\>\rm{T}\cdot 1\>\rm{m}\cdot\frac{c}{c} \nonumber \\
    &= e_0\cdot 1\>\frac{\rm{V}\,\rm{s}}{\rm{m}^2}\cdot 1\>\rm{m}\cdot \frac{3\cdot10^8\dfrac{\rm{m}}{\rm{s}}}{c} \nonumber \\
    &= 300\>\rm{MeV/c}\>.
\end{flalign}
%
...
%
\begin{flalign}
    \Delta t &= \frac{2\>\rm{m}}{3\cdot10^8\>\dfrac{\rm{m}}{\rm{s}}}\left[\sqrt{1 + \left(\frac{0.1396\>\rm{GeV}}{2\>\rm{GeV}}\right)^2} - \sqrt{1 + \left(\frac{0.4937\>\rm{GeV}}{2\>\rm{GeV}}\right)^2}\right] \nonumber \\
    &= 183\>\rm{ps}\>.
\end{flalign}
%
...
\begin{itemize}
    ...
    \end{equation}
    where $\sigma = \sigma_1 = \sigma_2$ or $\sigma = \max{(\sigma_1, \sigma_2)}$ and $\bar{x}_1$ and $\bar{x}_2$ are the means of the two peaks.
    ...
    \item \textbf{False Events} (\text{Lažni dogodki}) are events that are detected by the detector but are not actual
...
from $-\infty$ to $x_0$. In the context of data analysis $g(x)$ gives us the false positive rate while $f(x)$ gives us the
efficiency of the detector. The table is given in Table \ref{tab:gaussian-integral}.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.4\textwidth}
        ...
        \begin{tabular}{|l|l|l|}
            ...
            \multicolumn{1}{c|}{\cellcolor[HTML]{FFCCC9}$\bm{g(x_\mathrm{thr})}\>[\%]$} &
            \multicolumn{1}{c|}{\cellcolor[HTML]{FFCCC9}$\bm{f(x_\mathrm{thr})}\>[\%]$} \\ \hline
        ...
        \end{tabular}
        \captionof{table}{Table of the integral of the Gaussian distribution.}
        \label{tab:gaussian-integral}
    \end{minipage}
    \begin{minipage}{0.59\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../threshold.pdf}
        \caption{Visual representation of the Gaussian distributions and the threshold value.}
        \label{fig:example-figure}
    \end{minipage}\hfill
\end{figure}
Say we wanted to set the threshold value $x_\mathrm{thr}$ so that we can differentiate the first particle from the second with an efficiency of $99\%$. We read the width 
of the Gaussian distribution from the table where $f(x_\mathrm{thr}) = 99\%$ and set the threshold value to:
%
\begin{equation}
    x_\mathrm{thr} = x_1 + 2.327\cdot\sigma\>,
\end{equation}
%
where $x_1$ is the mean of the distribution of the first particle. We can also calculate the percentage of false events $F$ for this threshold value as:
%
\begin{equation}
    x_0' = x_2 - x_\mathrm{thr}\>,
\end{equation}
%
where $x_2$ is the mean of the distribution of the second particle. The percentage of false events is then:
%
\begin{equation}
    F = 1 - g(x_0')\>.
\end{equation}
%
If we want to set a threshold for a given rate of false events $F$ we can use the same formula as before where we replace the width of the Gaussian distribution for 
the threshold value $x_\mathrm{thr}$ with the value of $x_0'$ from the table where $g(x_0') = 1 - F$.
...
%
\begin{flalign}
    &x_0' = \rm{TOF}_K - \rm{TOF}_{\text{thr}} = 6.87\>\rm{ns} - 6.78\>\rm{ns} = 84\> \frac{\sigma}{60\rm{ps}}\rm{ps} = 1.4\cdot\sigma\>, \\
    &F = 1 - f(x_0') = 1 - 92\% = 8\%\>. \nonumber
\end{flalign}
%
...
%
\begin{flalign}
    D &= \frac{|\theta_\pi - \theta_K|}{\sigma_{\rm{track}}} \nonumber \\
    &= \frac{307.9\>\rm{mRad} - 285.3\>\rm{mRad}}{4.427\,\rm{mRad}} \nonumber \\
    &= 5.1\>.
\end{flalign}
...
We know we can measure the momentum of a particle using a magnetic field and the radius of curvature of the particle's trajectory.
The transverse momentum is given as:
%
\begin{equation}
    p_t = eBR\>,
\end{equation}
%
where $e$ is the charge of the particle, $B$ is the magnetic field and $R$ is the radius of curvature. Since the charge and the magnetic field are generally known
all we have to do is to determine the radius of curvature. During exercises we briefly went over the derivation of how this is done 
in a planar case where we have three measurements. We came to some intermediate results like:
%
\begin{flalign}
    &s = x_2 - \frac{x_1 + x_3}{2} \>, \\
    &s = \frac{e B L^2}{8p_t} \>, \\
    &\sigma_s^2 = \sigma_2^2 + \frac{\sigma_1^2}{4} + \frac{\sigma_3^2}{4} \Rightarrow \frac{6}{4} \sigma_x^2 \>.
\end{flalign}
%
These without a sketch of the geometry are relatively useless. To make things short, for three points, the uncertainty due to tracking is:
%
\begin{equation}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track} = \frac{8p_t}{eBL^2}\sigma_x \sqrt{\frac{3}{2}}\>,
    \label{eq:track-3}
\end{equation}
%
where $\sigma_x$ is the uncertainty of the measurement of the position of the particle. We see that the uncertainty is proportional to the momentum
of the particle. More general for a set of $N$ measurements we have:
%
\begin{equation}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track} = \frac{p_t}{eBL^2}\sigma_x \sqrt{\frac{720}{N+4}}\>.
    \label{eq:track-N}
\end{equation}
%
We must often also correct this due to multiple scattering. It is important to differentiate that $L$ in the context of tracking
means the distance of flight, while $L$ in the context of multiple scattering means the thickness of the material. In the event of a 
gas detector we can say that $d = L$. Thus the correction due to multiple scattering in a \textbf{semiconductor detector} is:
%
\begin{equation}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{MSC} = \frac{13.6\,\rm{MeV}}{c_0eBL}\sqrt{\frac{d}{X_0}}\>,
    \label{eq:msc-semi}
\end{equation} 
%
where $X_0$ is the radiation length of the material. In the event of a \textbf{gas detector} the correction is:
%
\begin{equation}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{MSC} = \frac{13.6\,\rm{MeV}}{c_0eB\sqrt{X_0 L}}\>.
    \label{eq:msc-gas}
\end{equation}
%
...
%
\begin{equation}
    \left(\frac{\sigma_{p_t}}{p_t}\right)^2 = \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track}^2 + \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{MSC}^2\>.
    \label{eq:sum-errors}
\end{equation}
%
\subsubsection{For a given semiconductor detector, which $p_t$ can we measure with a $10\%$ uncertainty?}
Lets imagine a semiconductor detector which uses silicon detectors with a thickness of $d=300\,\mu\rm{m}$, which 
allow a resolution of $\sigma_x = 10\,\mu\rm{m}$. The detector is made of $4$ such layers with a $L = 7.3\,\rm{cm}$ gap in between each
silicon layer. The radiation length of silicon is $X_0 = 9.37\,\rm{cm}$, while the magnetic field is $B=2\,\rm{T}$. \\

It is important for us to consider that we have $4$ layers of silicon detectors of thickness $d$ which give
us only $3$ gaps of size $L$. This also gives us $N=4$ measurements of position. Let us find the momentum $p_0$ for which the total uncertainty is at most $10\%$. We can first calculate 
the uncertainty due to multiple scattering using Equation (\ref{eq:msc-semi}), since the right-hand side of the equation is independent 
of momentum:
%
\begin{flalign}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{MSC} &= \frac{13.6\,\rm{MeV}}{eB3L}\sqrt{\frac{4d}{X_0}} \nonumber \\
    &= \frac{13.6\cdot 10^6\,\rm{eV}}{3\cdot10^8\,\dfrac{\rm{m}}{\rm{s}}\cdot\rm{e} \cdot 2\,\rm{T}\cdot 3\cdot 7.3\cdot 10^{-2}\,\rm{m}}\sqrt{\frac{4\cdot 300\cdot 10^{-6}\,\rm{m}}{9.37\cdot 10^{-2}\,\rm{m}}}\nonumber \\
    &= 0.0117 \approx 1.2\%\>,
\end{flalign}
%
where we've taken into account that $\rm{T} = \rm{Vs}/\rm{m}^2$ and added a $c$ to the denominator to get the units right (so we can use $eV$). From here 
we can use Equation (\ref{eq:sum-errors}) to calculate the uncertainty due to tracking:
%
\begin{flalign}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track}^2 &= \left(\frac{\sigma_{p_t}}{p_t}\right)^2 - \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{MSC}^2 = 0.1^2 - 0.012^2 \nonumber \\
    \Rightarrow \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track} &= 9.9\%\>.
\end{flalign}
%
From this we can now determine the momentum $p_0$ for which the total uncertainty is at most $10\%$:
%
\begin{flalign}
    p_0 &= \left(\dfrac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track}\frac{eB\cdot (3L)^2}{\sigma_x}\frac{1}{\sqrt{\dfrac{720}{N+4}}}\cdot c_0 \nonumber \\
    &= 0.099\cdot \frac{\rm{e}\cdot2\,\rm{T}\cdot\left(3\cdot7.3\cdot10^{-2}\,\rm{m}\right)^2}{10\cdot10^{-6}\,\rm{m}}\frac{1}{\sqrt{\dfrac{720}{4+4}}}\cdot 3\cdot10^8\,\dfrac{\rm{m}}{\rm{s}} \nonumber \\
    &= 30\,\rm{GeV/c}\>.
\end{flalign}
%
If we wanted to find the value of the momentum where both the uncertainty due to tracking and multiple scattering are equal we would have to set the two equal to each other and solve for $p_t$:
%
\begin{flalign}
    \frac{\sigma_x p_t}{c_0eBL^2}\sqrt{\frac{720}{N+4}} &= 0.0117 \nonumber \\
    \Rightarrow p_t &= 3.6\,\rm{GeV/c}\>.
\end{flalign}
%

\subsubsection{For the Central Drift Chamber used in the Belle experiment, what is the uncertainty of the momentum measurement for a $0.5\,\rm{GeV/c}$ particle?}
The Central Drift Chamber used in the Belle experiment is a gas detector which has 50 layers of anode wires in a space of $L=70\,\rm{cm}$. The gas used is 
a mixture of $50\%$ helium and $50\%$ butane by volume, which has a radiation length of $X_0 = 640\,\rm{m}$. The magnetic field is $B=1.5\,\rm{T}$. The spatial 
resolution of the detector is $\sigma_x = 130\,\mu\rm{m}$. \\

Since we are dealing with a gas detector we can use Equation (\ref{eq:msc-gas}) to calculate the uncertainty due to multiple scattering, where we've
already assumed that $d=L$:
%
\begin{flalign}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{MSC} &= \frac{13.6\,\rm{MeV}}{c_0eB\sqrt{X_0 L}} \nonumber \\
    &= \frac{13.6\cdot 10^6\,\rm{eV}}{3\cdot10^8\,\dfrac{\rm{m}}{\rm{s}}\cdot\rm{e}\cdot 1.5\,\rm{T}\cdot\sqrt{640\,\rm{m}\cdot 70\cdot 10^{-2}\,\rm{m}}}\nonumber \\
    &= 0.00143 \approx 0.14\%\>.
\end{flalign}
%
From here we can use Equation (\ref{eq:track-N}) to calculate the uncertainty due to tracking:
%
\begin{flalign}
    \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track} &= \frac{p_t}{eBL^2}\sigma_x \sqrt{\frac{720}{N+4}} \nonumber \\
    &= \frac{0.5\cdot10^9\,\rm{eV}}{3\cdot10^8\,\dfrac{\rm{m}}{\rm{s}}\cdot\rm{e}\cdot 1.5\,\rm{T}\cdot\left(70\cdot 10^{-2}\,\rm{m}\right)^2}\cdot 130\cdot 10^{-6}\,\rm{m}\sqrt{\frac{720}{50+4}} \nonumber \\
    &= 0.00107 \approx 0.11\%\>.
\end{flalign}
%
From this we can calculate the total uncertainty of the momentum measurement using Equation (\ref{eq:sum-errors}):
%
\begin{flalign}
    \left(\frac{\sigma_{p_t}}{p_t}\right)^2 &= \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track}^2 + \left(\frac{\sigma_{p_t}}{p_t}\right)_\mathrm{MSC}^2 = (0.00107)^2 + (0.00143)^2 = 0.000003189\nonumber \\
    \Rightarrow \left(\frac{\sigma_{p_t}}{p_t}\right) &= 0.18\%\>.
\end{flalign}
%
\subsection{Particle Tracking}
At this point during exercises we sketched how a particle would travel through $3$ dimensional space and which parameters of the track we'd 
have to determine to reconstruct the track. The general parametrization is given as:
%
\begin{flalign}
    x &= x_0 + R\left(\sin\phi - \sin\phi_0\right)\>, \\
    y &= y_0 - R\left(\cos\phi - \cos\phi_0\right)\>, \\
    z &= z_0 - R\cot\theta\left(\phi - \phi_0\right)\>,
\end{flalign}
%
where $R$ is the radius of curvature, $\theta$ is the azimuthal angle, $\theta_0$ is the azimuthal angle at the point of closest approach to the origin,
$\psi$ is the polar angle, $\psi_0$ is the polar angle at the point of closest approach to the origin, and $x_0$, $y_0$, $z_0$ are the coordinates of the point of closest approach to the origin.
From this we see that we have to determine $5$ parameters (the sixth can be determined from the others $x_0  = y_0/\tan\psi_0$). I was planning on adding an image of this but it is 
somewhat irrelevant since all practical exercises we did are in $1$ dimension. \\

In practice we fit these parameters to our measurements using various optimization methods. We had a look at two different methods:
\begin{itemize}
    \item \textbf{Least Squares Method} (\textit{Metoda najmanjših kvadratov}) where we minimize the sum of the squares of the residuals. This is a \textbf{global method} since we use all the data points at once.
    \item \textbf{Kalman Filter} (\textit{Kalmanov filter}) which is a \textbf{progressive method} where we update our estimate of the parameters as we get new data points.
\end{itemize}
%
Let's now imagine that we want to fit a linear model to a set of measurements. The scenario is presented in Figure \ref{fig:linear-fit}.
%
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{../linearfit.pdf}
    \caption{Fitting a linear model to a set of measurements.}
    \label{fig:linear-fit}
\end{figure}
%
We can write the model iteratively as:
%
\begin{equation}
    y_{n+1} = y_n + k_n\cdot\Delta x\>,
\end{equation}
%
where $k_n$ is the slope of the line after $n$ measurements. We will denote the uncertainty of the measurement with $\sigma$ and the uncertainty 
of the estimated parameter $k_n$ with $\sigma(k)$. Presented below are tables of how to calculate $k_n\Delta x$ and $\sigma(k)\Delta x$ for both of 
the methods. 

\begin{table}[H]
    \centering
        \large{\textbf{Kalman Filter}} \\[1ex]
        \begin{NiceTabular}{|l|l|l|}
            \hline
            \rowcolor[HTML]{EEB1F9}
            \multicolumn{1}{|c|}{\cellcolor[HTML]{EEB1F9}$\bm{n}$} &
            \multicolumn{1}{c|}{\cellcolor[HTML]{EEB1F9}\rule{0pt}{3ex}$\>\bm{k_n\Delta x}\>$} &
            \multicolumn{1}{c|}{\cellcolor[HTML]{EEB1F9}$\bm{\sigma(k)\Delta x}$ if $\bm{\sigma_i = \sigma}$} \\ \hline
            \hspace{0.5em}2\hspace{0.5em} & \multicolumn{1}{c}{\rule{0pt}{4ex}$y_2 - y_1$}\vspace{1ex} & \multicolumn{1}{c|}{$\sqrt{2}\,\sigma$} \\ \hline
            \hspace{0.5em}3\hspace{0.5em} & \multicolumn{1}{c}{\rule{0pt}{4ex}$\dfrac{1}{5}\left(3y_3 - y_2 - 2y_1\right)$} \vspace{1ex} & \multicolumn{1}{c|}{$\sqrt{\dfrac{14}{25}}\,\sigma = 0.748\cdot\sigma$} \\ \hline
            \hspace{0.5em}4\hspace{0.5em} & \multicolumn{1}{c}{\rule{0pt}{4ex}$\dfrac{1}{70}\left(30y_4 - y_3 - 18y_2 - 11y_1\right)$} \vspace{1ex} & \multicolumn{1}{c|}{$0.524\cdot\sigma$} \\ \hline
        \end{NiceTabular}
        \captionof{table}{Table for Kalman Filter.}
        \label{tab:kalman-table}
\end{table}

\begin{table}[H]
    \centering
        \large{\textbf{Least Squares Method}} \\[1ex]
        \begin{NiceTabular}{|l|l|l|}
            \hline
            \rowcolor[HTML]{FFADC3}
            \multicolumn{1}{|c|}{\cellcolor[HTML]{FFADC3}$\bm{n}$} &
            \multicolumn{1}{c|}{\cellcolor[HTML]{FFADC3}\rule{0pt}{3ex}$\>\bm{k_n\Delta x}\>$} &
            \multicolumn{1}{c|}{\cellcolor[HTML]{FFADC3}$\bm{\sigma(k)\Delta x}$ if $\bm{\sigma_i = \sigma}$} \\ \hline
            \hspace{0.5em}2\hspace{0.5em} & \multicolumn{1}{c}{\rule{0pt}{4ex}$y_2 - y_1$}\vspace{1ex} & \multicolumn{1}{c|}{$\sqrt{2}\,\sigma$} \\ \hline
            \hspace{0.5em}3\hspace{0.5em} & \multicolumn{1}{c}{\rule{0pt}{4ex}$\dfrac{1}{2}\left(y_3 - y_1\right)$} \vspace{1ex} & \multicolumn{1}{c|}{$\dfrac{1}{\sqrt{2}}\,\sigma = 0.707\cdot\sigma$} \\ \hline
            \hspace{0.5em}4\hspace{0.5em} & \multicolumn{1}{c}{\rule{0pt}{4ex}$\dfrac{1}{10}\left(3y_4 + y_3 - y_2 - 3y_1\right)$} \vspace{1ex} & \multicolumn{1}{c|}{$\dfrac{1}{\sqrt{5}}\,\sigma = 0.447\cdot\sigma$} \\ \hline
        \end{NiceTabular}
        \captionof{table}{Table for Least Squares Method.}
        \label{tab:least-squares-table}
\end{table}

The professor claims this is all that is needed for the exam, however one could also derive these from the general formulas for the
least squares method and the Kalman filter.

\subsubsection{For given measurements what is the relative error of the slope using the Kalman Filter and the Least Squares Method?}
We measured the position of a particle in three different planes which are seperated by a $10\,\rm{cm}$ gap. The measurements are:
%
\begin{flalign*}
    y_1 = 2.35\,\rm{mm}&\quad\sigma_1 = 0.05\,\rm{mm}\>, \\
    y_2 = 3.35\,\rm{mm}&\quad\sigma_2 = 0.05\,\rm{mm}\>, \\
    y_3 = 4.15\,\rm{mm}&\quad\sigma_3 = 0.1\,\rm{mm}\>.
\end{flalign*}
%
It is important to note here that the \textbf{uncertainty of the measurements} is \textbf{NOT the same} for all measurements. We will have to manually calculate 
the errors of the estimated slope for both methods. We can start by calculating $k_3\Delta x$ for the Kalman Filter:
%
\begin{flalign}
    k_3 &= \frac{1}{5\Delta x}\left(3y_3 - y_2 - 2y_1\right) \nonumber \\
    &= \frac{1}{5\cdot 100\,\rm{mm}}\left(3\cdot 4.15 - 3.35 - 2\cdot 2.35\right)\,\rm{mm} \nonumber \\
    &= 8.8\cdot 10^{-3}\>. \\
\end{flalign}
%
We can calculate the error by taking the square of each term with its respective error. This means:
%
\begin{equation}
    k_n\Delta x  = \frac{1}{\alpha}\left(\beta_n y_n +\dots+\beta_1 y_1\right) \quad\Rightarrow\quad \left(\sigma(k_n)\Delta x\right)^2 = \left(\frac{\beta_n}{\alpha}\sigma_n\right)^2+\dots+\left(\frac{\beta_1}{\alpha}\sigma_1\right)^2\>.
\end{equation}
%
So in our case we have:
%
\begin{flalign}
    \left(\sigma(k_3)\Delta x\right)^2 &= \left(\frac{3}{5}\sigma_3\right)^2 + \left(\frac{-1}{5}\sigma_2\right)^2 + \left(\frac{-2}{5\cdot}\sigma_1\right)^2 \nonumber \\
    &\Rightarrow \sigma(k_3) = 6.4\cdot10^{-4}\>.
\end{flalign}
%
From this we can find the relative of the slope for the Kalman Filter:
%
\begin{equation}
    \frac{\sigma(k_3)}{k_3} = \frac{6.4\cdot10^{-4}}{8.8\cdot10^{-3}} = 7.27\%\>.
\end{equation}
%
Analogue to this we can calculate the slope and its error for the Least Squares Method:
%
\begin{flalign}
    k_3 &= \frac{1}{2\Delta x}\left(y_3 - y_1\right) \nonumber \\
    &= \frac{1}{2\cdot 100\,\rm{mm}}\left(4.15 - 2.35\right)\,\rm{mm} \nonumber \\
    &= 9\cdot 10^{-3}\>. \\
\end{flalign}
%
...
%
\begin{flalign}
    \left(\sigma(k_3)\Delta x\right)^2 &= \left(\frac{1}{2}\sigma_3\right)^2 + \left(\frac{-1}{2}\sigma_1\right)^2 \nonumber \\
    &\Rightarrow \sigma(k_3) = 5.59\cdot10^{-4}\>.
\end{flalign}
%
From which we can get the relative error:
%
\begin{equation}
    \frac{\sigma(k_3)}{k_3} = \frac{5.59\cdot10^{-4}}{9\cdot10^{-3}} = 6.21\%\>.
\end{equation}
%
From this we can see that different optimization methods yield different results. 
The Kalman Filter gives us a relative error of $7.27\%$ while the Least Squares Method gives us a relative error of $6.21\%$.

\subsection{Bonus Exercises}
\subsubsection{Matching uncertainties in a Central Drift Chamber$^*$}
Consider that we are measuring the deposited energy and transverse momentum of protons using a Central Drift Chamber. The chamber 
is filled with Helium under normal conditions where it has the following properties:
%
\begin{flalign*}
    \rho &= 0.17\,\rm{g/L} = 0.00017\,\rm{g}/\rm{cm}^3\>, \\
    A &= 4\,\rm{g/mol}\>, \\
    Z &= 2\>, \\
    F &= 0.24\>, \\
    w &= 41.3\,\rm{eV}\>.
\end{flalign*}
%
The position detector system is made out of $25$ layers of anode wires. The dimension of this setup is $L=1\,\rm{m}$. The magnetic field inside is $B=2\,\rm{T}$.
What is the uncertainty of the spatial measurements in a Central Drift Chamber such that the 
uncertainty of the momentum measurement is the same as the uncertainty of the measured deposited energy if we are measuring a $5\,\rm{GeV/c}$ proton? \\

We first have to calculate the energy loss of the proton in such a system. Despite the fact that we have a meter of gas, this still counts as 
a thin absorber so we must use the Landau distribution from Equation (\ref{eq:landau}) to calculate the energy loss. We can check if this is alright later on.
For this we first need to calculate the velocity of the particle using Equation (\ref{eq:beta}):
%
\begin{flalign}
    \beta &= \frac{pc}{\sqrt{(pc)^2 + (Mc^2)^2}} \nonumber \\
    &= \frac{5\,\dfrac{\rm{GeV}}{c}\>c}{\sqrt{\left(5\,\dfrac{\rm{GeV}}{c}\>c\right)^2 + \left(0.938\,\dfrac{\rm{GeV}}{c^2}\>c^2\right)^2}} \approx 0.98285\>,
\end{flalign}
%
and the $\gamma$ factor:
%
\begin{flalign}
    \gamma &= \frac{1}{\sqrt{1 - \beta^2}} \approx 5.4228\>.
\end{flalign}
%
With this we can calculate $\xi$ which is given by Equation (\ref{eq:xi}):
%
\begin{flalign}
    \xi &= \frac{K}{2}\left\la\frac{Z}{A}\right\ra\frac{L\rho}{\beta^2} \nonumber \\
    &= \frac{0.3\,\dfrac{\rm{MeV}\rm{cm}^2}{\rm{g}}}{2}\cdot\frac{2}{4}\cdot\frac{100\,\rm{cm}\cdot0.00017\,\dfrac{\rm{g}}{\rm{cm}^3}}{\left(0.98285\right)^2} \nonumber \\
    &= 0.0013 \,\rm{MeV}\>,
\end{flalign}
%
and the mean excitation energy $I$ using Equation (\ref{eq:I}) for $Z<13$:
%
\begin{flalign}
    I &= Z\left(12 + \frac{7}{Z}\right)\,\rm{eV} \nonumber \\
    &= 2\left(12 + \frac{7}{2}\right)\,\rm{eV} = 31\,\rm{eV}\>.
\end{flalign}
%
We can also calculate our approximation for the maximum energy transfer in a single collision $W_{\text{max}}$ using Equation (\ref{eq:wmax}):
%
\begin{flalign}
    W_{\text{max}} &= 2m_ec^2\beta^2\gamma^2 \nonumber \\
    &= 2\cdot0.511\,\rm{MeV}\cdot(0.98285)^2\cdot(5.4228)^2 \approx 29\,\rm{MeV}\>.
\end{flalign}
%
Now we can calculate the energy loss using the Landau distribution:
%
\begin{flalign}
    \Delta_p &= \xi\left[\ln{\frac{2m_ec^2\beta^2\gamma^2}{I}} + \ln{\frac{\xi}{I} + j - \beta^2}\right] \nonumber \\
    &= 0.0013\,\rm{MeV}\left[\ln{\frac{29\cdot10^6\,\rm{eV}}{31\,\rm{eV}}} + \ln\frac{0.0013\cdot10^6\,\rm{eV}}{31\,\rm{eV}} + 0.2 -(0.98285)^2\right] \nonumber \\
    &= 0.0013\,\rm{MeV}\cdot 16.719 \nonumber \\
    &= 0.022\,\rm{MeV}\>.
\end{flalign}
%
From this we can calculate the number of produced electron-ion pairs $N$ using Equation (\ref{eq:ion-N}):
%
\begin{flalign}
    N &= \frac{\Delta_p}{w} \nonumber \\
    &= \frac{0.022\cdot10^6\,\rm{eV}}{41.3\,\rm{eV}} \approx 532\>.
\end{flalign}
%
Thus we can then use Equation (\ref{eq:ionization-res}) to calculate the resolution for the deposited energy:
%
\begin{flalign}
    R = \frac{\sqrt{F\cdot N}}{N} = \frac{\sqrt{0.24\cdot 532}}{532} \approx 2.1\%\>.
\end{flalign}
%
Now we can use the equation for tracking resolution Equation (\ref{eq:track-N}) to calculate the required spatial measurement uncertainty:
%
\begin{flalign}
    \sigma_x &= \frac{eBL^2\left(\dfrac{\sigma_{p_t}}{p_t}\right)_\mathrm{Track}}{p_t}\cdot\left[\frac{720}{N+4}\right]^{-\frac{1}{2}} \nonumber \\
    &= \frac{3\cdot10^8\,\dfrac{\rm{m}}{\rm{s}}\cdot \rm{e}\cdot2\,\rm{T}\cdot(1\,\rm{m})^2\cdot 0.021}{5\cdot10^9\,\rm{eV}}\cdot\left[\frac{720}{25+4}\right]^{-\frac{1}{2}} \nonumber \\
    &= 0.0048\,\rm{m} = 4.8\,\rm{mm}\>.
\end{flalign}
